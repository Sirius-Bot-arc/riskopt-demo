  ...
  return (
    <div key={org.id} className="page-enter flex flex-col gap-5">
      <section className="optimizer-hero">
        ...
        <div className="relative max-w-4xl"><div className="eyebrow"><span className="eyebrow-dot" /> DECISION LAB · {org.name.toUpperCase()}</div><h2 className="optimizer-title">If you had <span>{formatINRShort(budget)}</span> to spend today,<br />where would you put it?</h2><p className="hero-copy max-w-2xl">RiskOpt tests the control mix against your budget and turns a pile of cyber risks into one defensible investment plan.</p>
          <div className="hero-chips"><span>✦ AI-guided</span><span>◌ Budget-aware</span><span>♡ Explainable</span></div></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[20rem_1fr]">
        <aside className="flex flex-col gap-4">
          ...
          <Card className="premium-card"><CardHeader><p className="section-kicker">02 · CAPITAL</p><CardTitle className="mt-1 text-lg">Investment ceiling</CardTitle><p className="budget-helper">Slide it around — RiskOpt adapts the plan to your reality.</p></CardHeader><CardContent><div className="budget-readout"><span>{formatINRShort(budget)}</span><small>{formatINR(budget)}</small></div><input type="range" min={org.budgetMin} max={org.budgetMax} step={step} value={budget} onChange={(e) => setBudget(Number(e.target.value))} className="budget-range" aria-label="Budget" /><div className="flex justify-between text-[11px] font-medium text-slate-400"><span>{formatINRShort(org.budgetMin)}</span><span>{formatINRShort(org.budgetMax)}</span></div><Button onClick={runOptimizer} disabled={running} className="optimizer-run mt-5 h-12 w-full rounded-2xl shadow-lg shadow-violet-200/60">{running ? <><Loader2 className="size-4 animate-spin" /> Thinking...</> : <><WandSparkles className="size-4" /> Optimize my budget <span className="button-spark">✦</span></>}</Button></CardContent></Card>
        </aside>
